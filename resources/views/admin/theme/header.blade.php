        ...
        <div class="px-md-3 px-0 d-flex align-items-center">
            ...
            @if (@Auth::user()->type == 1)
                @if (helper::check_restaurant_closed() == 1)
                    ...
                    <input id="open-close-switch" type="checkbox" class="checkbox-switch" name="open-close"
                        value="1" checked
                        @if (env('Environment') == 'sendbox') onclick="myFunction()" disabled @else onclick="changeStatus(2,'{{ URL::to('admin/change-status') }}')" @endif>
                @else
                    @php
                    ...
                    <input id="open-close-switch" type="checkbox" class="checkbox-switch" name="open-close"
                        value=""
                        @if (env('Environment') == 'sendbox') onclick="myFunction()" disabled @else onclick="changeStatus(1,'{{ URL::to('admin/change-status') }}')" @endif>
                @endif
            ...
            @if (@helper::checkaddons('language'))
                <div class="position-relative mx-1">
                    <div class="lag-btn dropdown d-lg-block d-none">
                        <a class="btn btn-sm d-flex align-items-center gap-2 border-primary" href="javascript:void(0)" role="button"
                            data-bs-toggle="dropdown" aria-expanded="false">
                            <img src="{{ helper::image_path(session()->get('flag')) }}" alt=""
                                class="rounded-circle language-width">
                            <span
                                class="{{ Session::get('theme') == 'dark' ? 'text-white' : '' }}">{{ session()->get('language') }}
                            </span>
                        </a>
                        <ul
                            class="dropdown-menu p-0 mt-1 bg-body-secondary shadow border-0 {{ session()->get('direction') == 2 ? 'drop-menu-rtl' : 'drop-menu-ltr' }}">
                    ...
                    <!-- language-btn -->
                    <div class="lag-btn dropdown d-block d-lg-none">
                        <a class="btn text-dark border fs-6" type="button" id="dropdownMenuButton1"
                            data-bs-toggle="dropdown" aria-expanded="false">
                            <i class="fa-solid fa-globe fs-5"></i></a>
                        <ul class="dropdown-menu p-0 mt-1 bg-body-secondary shadow border-0 {{ session()->get('direction') == '2' ? 'min-dropdown-rtl' : 'min-dropdown-ltr' }}"
