...
export default function BranchesClient({ initialBranches, error }) {
  ...
  const handleAddBranch = async (e) => {
    ...
    try {
      ...
      if (data.status === 1) {
        ...
        setFormError(data.error || "Failed to add branch");
      }
    } catch (err) {
  ...
  const handleEditBranch = async (e) => {
    ...
    try {
      ...
      if (data.status === 1) {
        setBranches(
          branches.map((b) => (b.id === selectedBranch.id ? data.branch : b))
        );
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    try {
      ...
      if (data.status === 1) {
        setBranches(
          branches.map((b) => (b.id === id ? { ...b, status: nextStatus } : b))
        );
      } else {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[#1c1c1c] text-xs">
              {filtered.map((br) => (
                <tr key={br.id} className="hover:bg-[#161616]/40 transition-colors group">
                  ...
                  <td className="py-4 px-6">
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5 text-gray-300">
                        ...
                        <span>{br.phone || "N/A"}</span>
                      </div>
                      <div className="flex items-center gap-1.5 text-gray-400">
                        <i className="fas fa-envelope text-[10px] text-gray-500"></i>
                        <span>{br.email || "N/A"}</span>
                      </div>
                    </div>
                  </td>
                  <td className="py-4 px-6 text-gray-400">
                    {br.address || <span className="text-gray-600 italic">No address provided</span>}
                  </td>
                  <td className="py-4 px-6">
                    <button
                      onClick={() => handleToggleStatus(br.id, br.status)}
                    ...
                    >
                      {br.status === 1 ? "Active" : "Inactive"}
                    </button>
                  </td>
                  <td className="py-4 px-6 text-right space-x-2">
                    <button
                      onClick={() => handleOpenEditModal(br)}
          ...
          <div className="relative bg-[#111111] border border-[#222] rounded-2xl w-full max-w-lg overflow-hidden flex flex-col z-10">
            <div className="p-5 border-b border-[#222] flex items-center justify-between">
              <h3 className="text-base font-bold flex items-center gap-2">
                <i className="fas fa-store text-[#00e676]"></i> Add New Branch
              </h3>
            ...
            <form onSubmit={handleAddBranch} className="p-6 space-y-4 overflow-y-auto max-h-[70vh]">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    placeholder="e.g. downtown@foodefy.com"
                    className="w-full px-4 py-2.5 bg-[#050505] border border-[#222] rounded-xl text-white placeholder-gray-600 focus:outline-none focus:border-[#00e676]/50 text-sm"
                  />
                </div>
              ...
              <div className="pt-4 flex gap-3 border-t border-[#222] mt-6">
                ...
                >
                  {loading ? <i className="fas fa-spinner fa-spin"></i> : "Create Branch"}
                </button>
              </div>
            </form>
          </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#111111] border border-[#222] rounded-2xl w-full max-w-lg overflow-hidden flex flex-col z-10">
            <div className="p-5 border-b border-[#222] flex items-center justify-between">
              <h3 className="text-base font-bold flex items-center gap-2">
                <i className="fas fa-edit text-[#00e676]"></i> Edit Branch
              </h3>
            ...
            <form onSubmit={handleEditBranch} className="p-6 space-y-4 overflow-y-auto max-h-[70vh]">
              <div className="grid grid-cols-2 gap-4">
                <div className="form-group col-span-2">
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">
                    Branch Name *
